...
const std::map<std::string_view, ArgInfo> arguments{
    ...
    { "envelope",
        {
            .name{ "--[envelope]" },
            .key{ "<amp> <behavior> [width <wx> <wy>] [pos <px> <py>] [pol <pol>] [exponent <n>] [charge <m>] [type <type>] [momenta <k0x> <k0y>] [time <kind> <t0> <sigma> <freq>]" },
            .short_description{ "Envelope syntax for --pump, --pulse, --potential, --fftMask, --initialState, --initialReservoir. Amplitude and behaviour are required and positional; all other parameters are optional named keywords in any order. Defaults: pos=(0,0), pol=both, exponent=1, charge=none, type=gauss+noDivide, momenta=(0,0), no temporal (constant)." },
            .long_description{ "Envelope syntax used by --pump, --pulse, --potential, --fftMask, --initialState, --initialReservoir.\nRequired (positional):\n  amp: Amplitude.\n  behavior: How to combine with other envelopes in the same temporal group: add, multiply, replace, adaptive (scale by current matrix value), complex (multiply by i).\nOptional named keywords (any order):\n  width <wx> <wy>: Gaussian width in x and y. Omitting gives a point-like envelope.\n  pos <px> <py>: Center position. Default: 0 0.\n  pol <p>: Polarization - plus, minus, or both. Default: both.\n  exponent <n>: Gaussian exponent (larger = steeper). Default: 1.\n  charge <m>: Topological charge, adds phase winding e^(i*m*phi). Use integer or none. Default: none.\n  type <t>: Shape flags joined with +: gauss, ring, noDivide, outerExponent, local. Default: gauss+noDivide.\n  momenta <k0x> <k0y>: Momentum kick, multiplies envelope by exp(i*(k0x*(x-x0)+k0y*(y-y0))). Default: 0 0.\n  time <kind> <t0> <sigma> <freq>: Temporal envelope. kind = iexp (~exp(i*freq*t)*gauss), cos (~cos(freq*t)*gauss), gauss (~gauss^freq). Default: constant.\nLegacy positional syntax is also accepted: <amp> <behavior> <wx> <wy> <px> <py> <pol> <exp> <charge> <type> [momenta <k0x> <k0y>] [time <kind> <t0> <sigma> <freq>]." },
            .short_usecase{ "--[envelope] 15 add width 10 10" },
            .long_usecase{ "--[envelope] 15 add width 10 10 (basic Gaussian at origin)\n--[envelope] 15 add width 20 10 pos 50 0 type ring+noDivide exponent 3 (steeper ring, asymmetric, shifted to x=50)\n--[envelope] 1 add width 0.3 0.3 pol plus type local+noDivide (local-mapped envelope -1:1, plus component only)\n--[envelope] 15 add width 2 2 momenta 1.0 0.5 (Gaussian with momentum kick, adds phase exp(i*(k0x*x+k0y*y)))\n--[envelope] 15 add width 10 10 time iexp 200 50 0.1 (time-dependent with complex oscillator)\n--[envelope] 1 add width 2 2 momenta 1 0 time iexp 0 1e5 0.1 (propagating pulse with momentum and temporal envelope)" }
        }
    },
    { "envelope_time",
        {
            .name{ "--[envelope] (legacy time-dependent syntax)" },
            .key{ "<amp> <behavior> <wx> <wy> <px> <py> <pol> <exp> <charge> <type> [momenta <k0x> <k0y>] time <kind> <t0> <sigma> <freq>" },
            .short_description{ "Legacy positional syntax for time-dependent envelopes. Prefer the named-keyword syntax above." },
            .long_description{ "Legacy positional time-dependent syntax. All spatial parameters must be given in fixed order before the time keyword. Prefer the named-keyword syntax which allows omitting parameters and specifying them in any order.\ntime kind options: iexp ~exp(i*freq*(t-t0))*gauss(t0,sigma), cos ~cos(freq*(t-t0))*gauss(t0,sigma), gauss ~gauss(t0,sigma)^freq." },
            .short_usecase{ "--[envelope] 15 add 10 10 0 0 both 1 none gauss+noDivide time iexp 200 50 0.1" },
            .long_usecase{ "--[envelope] 15 add 10 10 0 0 both 1 none gauss+noDivide time iexp 200 50 0 (complex oscillator at t=200, width=50, no frequency)\n--[envelope] 15 add 10 10 0 0 both 1 3 gauss+noDivide time iexp 0 1e5 0.1 (topological charge 3, quasi-constant temporal, f=0.1)" }
        }
    },
    { "envelope_loaded",
        {
